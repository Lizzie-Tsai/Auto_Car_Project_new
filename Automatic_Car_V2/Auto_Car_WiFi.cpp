...
void WiFi_STA_Disconnected_Handler(WiFiEvent_t wifi_event, WiFiEventInfo_t wifi_info) {
  Serial.println("Disconnected From WiFi STA Network");
  // Resatrt ESP32
  ESP.restart();
}
void WiFi_AP_Disconnected_Handler(WiFiEvent_t wifi_event, WiFiEventInfo_t wifi_info) {
  Serial.println("Disconnected From WiFi AP Network");
  // Resatrt ESP32
  ESP.restart();
}
...
void WiFi_Setup(bool WiFi_Mode)
{
  if (WiFi_Mode == 0)
  {
    WiFi_MODE = 0;
    WiFi.mode(WIFI_STA);
    WiFi.onEvent(WiFi_STA_Disconnected_Handler, ARDUINO_EVENT_WIFI_STA_DISCONNECTED);
  ...
  else
  {
    ...
    WiFi.mode(WIFI_AP);
    WiFi.onEvent(WiFi_AP_Disconnected_Handler, ARDUINO_EVENT_WIFI_AP_STADISCONNECTED);
...
//       {
//         wtdFlag = 0;
//         Serial.println("WiFi WTD not feed");
//         ESP.restart();
//       }
//     }
//     else

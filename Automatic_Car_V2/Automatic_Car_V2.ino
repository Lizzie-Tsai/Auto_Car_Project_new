...
#include "Auto_Car_WiFi.h"
#include "Auto_Car_Emotion.h"
#include "Auto_Car_WS2812.h"
#include "Auto_Car_For_ESP32.h"
...
void setup() {
  ...
  // Set alarm to call onTimer function every 3 seconds (value in microseconds).
  // Repeat the alarm (third parameter) with unlimited count = 0 (fourth parameter).
  timerAlarm(Timer0_Cfg, 3000000, true, 0);

  //xTaskCreateUniversal(loopTask_Camera, "loopTask_Camera", 8192, NULL, 0, NULL, 0);
  xTaskCreateUniversal(loopTask_WTD, "loopTask_WTD", 8192, NULL, 0, NULL, 4);
...
void loop() {
  ...
  if (client) {                                               //if you get a client
    ...
    while (client.connected()) {                              //loop while the client's connected    
      ...
      WiFi_send_updates(client, send_updates_flag);//send updates if timer expired
    }
...
void BT_EventHandler(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) {
  ...
  if (event == ESP_SPP_START_EVT) {
    ...
    while (SerialBT.available()) {
      ...
      if(message==turnON){
        WS2812_SetMode(4);     //Car color lights display function
        Serial.println(F(" :LED ON"));
        SerialBT.println(F("LED ON"));   
      }
      else if(message==turnOFF){
        WS2812_SetMode(0);        //Turn LED Off
...
void start_Line_Track(void *pvParameters){
  ...
  while(1){
    ...
    if (xSemaphoreTake(mutex, 1/portTICK_PERIOD_MS) == pdTRUE){
      if((xEventGroupGetBits(xEventGroup) & (TB_BIT)) != 0){

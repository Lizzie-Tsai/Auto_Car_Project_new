...
from Command import STATE as state
from Command import STATE_STRING as state_s
from Command import CAUSE_STRING as cause_s
...
class mywindow(QMainWindow,Ui_Client):
    def __init__(self):
        ...
        self.label_Video.setPixmap(QPixmap('image/road.png'))
        ...
        self.Override_1.clicked.connect(self.on_btn_Override_1)
        self.Override_2.clicked.connect(self.on_btn_Override_2)
        ...
        self.checkBox_Led_Mode1.setEnabled(False)
        self.checkBox_Led_Mode1.setStyleSheet("color: #302f2f;") 
        self.checkBox_Led_Mode2.setEnabled(False)
        self.checkBox_Led_Mode2.setStyleSheet("color: #302f2f;")
        self.checkBox_Led_Mode3.setEnabled(False)
        self.checkBox_Led_Mode3.setStyleSheet("color: #302f2f;")
        self.checkBox_Led_Mode4.setEnabled(False)
        self.checkBox_Led_Mode4.setStyleSheet("color: #302f2f;")
        self.checkBox_Matrix_Mode1.setEnabled(False)
        self.checkBox_Matrix_Mode1.setStyleSheet("color: #302f2f;")
        self.checkBox_Matrix_Mode2.setEnabled(False)
        self.checkBox_Matrix_Mode2.setStyleSheet("color: #302f2f;")
        self.checkBox_Matrix_Mode3.setEnabled(False)
        self.checkBox_Matrix_Mode3.setStyleSheet("color: #302f2f;")
        self.checkBox_Matrix_Mode4.setEnabled(False)
        self.checkBox_Matrix_Mode4.setStyleSheet("color: #302f2f;")
        self.Btn_ForWard.setEnabled(False)
        self.Btn_ForWard.setStyleSheet("color: #302f2f;")
        self.Btn_BackWard.setEnabled(False)
        self.Btn_BackWard.setStyleSheet("color: #302f2f;")
        self.Btn_Turn_Left.setEnabled(False)
        self.Btn_Turn_Left.setStyleSheet("color: #302f2f;")
        self.Btn_Turn_Right.setEnabled(False)
        self.Btn_Turn_Right.setStyleSheet("color: #302f2f;")
        self.Btn_Buzzer.setEnabled(False)
        self.Btn_Buzzer.setStyleSheet("color: #302f2f;")
    ...
    def keyPressEvent(self, event):
        ...
        if (event.key() == Qt.Key_Y):
            self.on_btn_Override_1()
        if (event.key() == Qt.Key_U):
            self.on_btn_Override_2()
    ...
    def on_btn_Track(self):#Tracking car control
        if self.commandFlag:
            if self.trackFlag==0:
                ...
                if self.cat_State == "/OVERRIDE 1":
                    self.checkBox_Led_Mode1.setEnabled(True)
                    self.checkBox_Led_Mode1.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Led_Mode2.setEnabled(True)
                    self.checkBox_Led_Mode2.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Led_Mode3.setEnabled(True)
                    self.checkBox_Led_Mode3.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Led_Mode4.setEnabled(True)
                    self.checkBox_Led_Mode4.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Matrix_Mode1.setEnabled(True)
                    self.checkBox_Matrix_Mode1.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Matrix_Mode2.setEnabled(True)
                    self.checkBox_Matrix_Mode2.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Matrix_Mode3.setEnabled(True)
                    self.checkBox_Matrix_Mode3.setStyleSheet("color: #DCDCDC;")
                    self.checkBox_Matrix_Mode4.setEnabled(True)
                    self.checkBox_Matrix_Mode4.setStyleSheet("color: #DCDCDC;")
                    self.Btn_ForWard.setEnabled(False)
                    self.Btn_ForWard.setStyleSheet("color: #302f2f;")
                    self.Btn_BackWard.setEnabled(False)
                    self.Btn_BackWard.setStyleSheet("color: #302f2f;")
                ...
                self.Btn_ForWard.setStyleSheet("color: #DCDCDC;")
                self.Btn_BackWard.setEnabled(True)
                self.Btn_BackWard.setStyleSheet("color: #DCDCDC;")
                self.Btn_Turn_Left.setEnabled(True)
                self.Btn_Turn_Left.setStyleSheet("color: #DCDCDC;")
                self.Btn_Turn_Right.setEnabled(True)
                self.Btn_Turn_Right.setStyleSheet("color: #DCDCDC;")
                self.Btn_Buzzer.setEnabled(True)
                self.Btn_Buzzer.setStyleSheet("color: #DCDCDC;")
    def on_btn_Light(self):#Light seeking car control
        if self.commandFlag:
            if self.lightFlag==0:
                ...
                self.Btn_Buzzer.setEnabled(True)
                self.Btn_Buzzer.setStyleSheet("color: #DCDCDC;")
    def on_btn_Override_1(self):
        # self.targetBtn.setEnabled(False)
        if self.cat_State == "":
            self.cat_State = "/OVERRIDE 1"
            self.checkBox_Led_Mode1.setEnabled(True)
            self.checkBox_Led_Mode1.setStyleSheet("color: #DCDCDC;")
            self.checkBox_Led_Mode2.setEnabled(True)
            self.checkBox_Led_Mode2.setStyleSheet("color: #DCDCDC;")
            ...
            self.cat_State = ""
            if (self.lightFlag == 1 or self.trackFlag == 1):
                self.checkBox_Led_Mode1.setEnabled(False)
                self.checkBox_Led_Mode1.setStyleSheet("color: #302f2f;") 
                self.checkBox_Led_Mode2.setEnabled(False)
                self.checkBox_Led_Mode2.setStyleSheet("color: #302f2f;")
    ...
    def on_btn_Override_2(self):
        self.cat_State = ""
        self.lightFlag = 0
        self.trackFlag = 0
        self.TCP.sendData(cmd.CMD_OVERRIDE+self.endChar)
    ...
    def recvmassage(self):
            ...
            while True:
                ...
                for oneCmd in cmdArray:
                    ...
                    if cmd. CMD_UPDATES in Massage:
                        if float(Massage[1]) >= 0:
                            percent_light=int(float(Massage[1]))
                        else:
                            percent_light = 0
                        self.label_Light_Sensor.setText(str(percent_light))

                        if str(Massage[2]) == state.STATE_O:
                            state_string = state_s.IN_MOTION_O + self.cat_State
                            cause_string = cause_s.NONE
                        elif str(Massage[2]) == state.STATE_T:
                            state_string = state_s.IN_MOTION_T + self.cat_State
                            cause_string = cause_s.NONE
                        elif str(Massage[2]) == state.STATE_OS:
                            state_string = state_s.MOTIONLESS + self.cat_State
                            cause_string = cause_s.OS
                        elif str(Massage[2]) == state.STATE_TS:
                            state_string = state_s.MOTIONLESS + self.cat_State
                            cause_string = cause_s.TS
                        elif str(Massage[2]) == state.STATE_BS:
                            state_string = state_s.MOTIONLESS + self.cat_State
                            cause_string = cause_s.BS
                        elif str(Massage[2]) == state.STATE_RS:
                            if  self.cat_State == "/OVERRIDE 1":
                                state_string = self.cat_State[1:]
                                cause_string = cause_s.RS
                            else:
                                state_string = state_s.OVERRIDE_2
                                cause_string = cause_s.RS
                        else:
                            if  self.cat_State == "/OVERRIDE 1":
                                state_string = self.cat_State[1:]
                                ...
                                state_string = state_s.NONE
                                cause_string = cause_s.NONE
                        self.State_Value.setText(state_string)
                        self.Cause_Stop_Value.setText(cause_string)
